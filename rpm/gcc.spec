...
%global build_ada 0
%global build_objc 0
%global build_go %{?_gcc_build_go}%{!?_gcc_build_go:0}
%global build_d 0
...
URL: https://github.com/sailfishos/gcc
...
echo 'Sailfish OS gcc %{gcc_version}-%{gcc_release}' > gcc/DEV-PHASE
...
%else
	--disable-bootstrap \
%endif
	--with-bugurl=https://github.com/sailfishos/ \
...
%endif
	...
	--disable-libmpx \
%if %{bootstrap} == 0
	--enable-languages=c,c++,lto${enablelgo} \
...
for d in %{buildroot}%{_libdir} %{buildroot}/%{_lib} \
	  ...
	  %{buildroot}%{_prefix}/include/c++/%{gcc_version}/%{gcc_target_platform}/%{_lib}; do
  mkdir -p $d
  (cd $d && ln -sf . lp64d)
done
%endif
...
echo gcc-%{gcc_version}.%{_arch} > $FULLPATH/rpmver
...
%{_prefix}/bin/%{gcc_target_platform}-gcc-%{gcc_version}
...
%endif
%if %{build_libhwasan}
%{_prefix}/%{_lib}/gcc/%{gcc_target_platform}/%{gcc_version}/libhwasan.so
%{_prefix}/%{_lib}/gcc/%{gcc_target_platform}/%{gcc_version}/libhwasan_preinit.o
%endif
%if %{build_liblsan}
...
%dir %{_prefix}/%{_lib}/gcc/%{gcc_target_platform}
%dir %{_prefix}/%{_lib}/gcc/%{gcc_target_platform}/%{gcc_version}
%{_prefix}/%{_lib}/gcc/%{gcc_target_platform}/%{gcc_version}/libhwasan.a
%{!?_licensedir:%global license %%doc}
%license libsanitizer/LICENSE.TXT
%endif
